...
const account1 = {
  ...
  movementsDates: [
    ...
    '2026-03-12T23:36:17.929Z',
    '2026-03-16T10:51:36.790Z'
  ],
  currency: 'EUR',
  locale: 'pt-PT' // de-DE
};
...
const calcDisplaySummary = function (acc) {
  ...
  const incomes = movement.filter((mov) => mov > 0).reduce((acc, mov) => acc + mov, 0);
...
const converTitleCase = function (title) {
  ...
  const exceptionss = ['a', 'an', 'and', 'but', 'the', 'or', 'on', 'in', 'with'];
  const titleCase = title
    .toLowerCase()
    .split(' ')
    .map((word) => (exceptionss.includes(word) ? word : capitalize(word)))
    .join(' ');
  return capitalize(titleCase);
};
console.log(converTitleCase('and this is a nice title'));
